...
export default async (policyContext, config, { strapi }: { strapi: Core.Strapi }) => {
  ...
  try {
    ...
    if (userWithTenant?.tenants) {
       ...
       // 4. (Opcional, mas recomendado) Inject tenantId into state for easier access in controllers/services
       policyContext.state.tenantId = tenantId; // Ou use ctx.state.user.tenant = userWithTenant.tenant;

       // Você também pode modificar ctx.query aqui para adicionar filtros de tenant automaticamente
       // Exemplo:
       // if (!policyContext.query) policyContext.query = {};
       // if (!policyContext.query.filters) policyContext.query.filters = {};
       // policyContext.query.filters.tenant = tenantId;
       // strapi.log.debug(`isOwnerTenant: Added tenant filter ${tenantId} to query.`);

       return true; // User belongs to a tenant, allow access
    } else {
      strapi.log.warn(`isOwnerTenant: User ${user.id} does not belong to a tenant.`);
      return false; // User does not have a tenant, deny access (or return 403 Forbidden)
    }
  } catch (error) {
