        ...
        >
            <motion.div
                ...
                <span className='absolute overflow-hidden inset-0 z-[-1]'>
                    <StrapiImage src={backgroundImage.url} alt={title} fill className=" w-full object-cover  group-hover:scale-105 transition-transform duration-1000 z-0 brightness-50" />
                ...
                <div className='absolute inset-0 overflow-hidden' >
                    <span
                        className='text-2xl xl:text-8xl scale-90  text-center font-rustic group-hover:-translate-y-[70%] rounded-b-none group-hover:text-primary-500/0 transition-all duration-1000  text-primary-500 bg-black p-4 h-full w-full flex justify-center items-center'
                    >
                        {title}
                    ...
                    <span
                        className='absolute inset-x-8 rounded-t-full bottom-0 text-md xl:text-xl  text-center font-semibold  translate-y-full group-hover:translate-y-0  group-hover:text-white text-black/0 transition-all duration-1000   bg-black p-4 flex flex-col space-y-4 justify-center items-center'
                    >

                        <h3 className='text-muted-foreground font-thin '>Contém:</h3>
                        <div className='flex space-x-2'>
                            {categories.map((category: any, index: number) => (
                                <div className="p-1 bg-primary-500 text-black text-sm font-thin rounded-full" key={index}>
                                    {category.title}
                                </div>
                            ))}
                        </div>
